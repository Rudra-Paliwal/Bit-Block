...
    let pkg;
    try {
      // IMPORTANT: Read the ciphertext from the input box (plainEl)
      pkg = JSON.parse(plainEl.value.trim()); // <--- CHANGED: Reading from plainEl
    } catch (e) {
      flash('Input is not a valid JSON package.', 'error'); // <--- Label changed for clarity
      return;
    }
...
      
      // IMPORTANT: Write the decrypted plaintext to the output box (outEl)
      outEl.textContent = dec.decode(pt); // <--- CHANGED: Writing to outEl
      
